...
class EdirectCommentsCommentsModuleFrontController extends ModuleFrontController
{
	public $product;
	...
	public function setMedia()
	{
		...
		$this->path = __PS_BASE_URI__.'modules/edirectcomments/';
		...
		$this->context->controller->addCSS($this->path.'views/css/edirectcomments.css', 'all');
		$this->context->controller->addJS($this->path.'views/js/edirectcomments.js');
	}

	protected function initList()
	{
		// Get number of comments
		$nb_comments = EdirectComment::getProductNbComments($this->product->id);
		...
		// Get comments
		$comments = EdirectComment::getProductComments($this->product->id, $limit_start, $limit_end);
