...
class Search extends SearchCore
{
								...
								$order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
	{
		...
		if (isset($find['result']) && !empty($find['result']) && Module::isInstalled('edirectcomments'))
		{
			// List id product
			...
			require_once(dirname(__FILE__).'/../../modules/edirectcomments/classes/EdirectComment.php');
			$grades_comments = EdirectComment::getInfosOnProductsList($id_product_list);
			...
			foreach ($products as $kp => $p)
				foreach ($grades_comments as $gc)
					...
					{
						$products[$kp]['edirectcomments']['grade_avg'] = round($gc['grade_avg']);
						$products[$kp]['edirectcomments']['nb_comments'] = $gc['nb_comments'];
					}
